...
<div class="container border border-dark border-3">
    ...
    <div class="row border border-dark border-3">
        <div class="row">
            ...
            <form action="Edit" method="POST" id="nameForm">
                <div class="shadow-lg">
                    <div class="accordion-item row-cols-3">
                        <label for="bookCode">Book Code :: </label>
                        <label class="accordion-body" id="bookCode">${thisBookCode}</label>
                        <input type="hidden" name="editCode" value="${thisBookCode}">
                    </div>
                    <div class="accordion-item row-cols-3">
                        <label for="bookNameField">Book Name :: </label>
                        <input class="accordion-body" name="bookName" id="bookNameField"
                               placeholder="Enter Book Name", value="${thisBookName}">
                    </div>
                    <div class="accordion-item row-cols-3 d-flex d-inline">
                        <label class="mt-3" for="bookNameField">Select Author :: </label>
                        <select class="form-select accordion-body" name="bookAuthor"
                                aria-label="Default select example" style="width: 33.8%;">
                            <c:forEach var="auth" items="${authors}">
                                <option value="${auth.authorName}">${auth.authorName}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="accordion-item row-cols-3">
                        <label for="dateToady">Added On :: </label>
                        <label class="accordion-body" id="dateToady">${dateBookAdded}</label>
                        <input type="hidden" name="getDateToday" value="${dateBookAdded}">
                    </div>
